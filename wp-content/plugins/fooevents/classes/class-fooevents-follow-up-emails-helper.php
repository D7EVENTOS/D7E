...
class FooEvents_Follow_Up_Emails_Helper {
	...
	public function add_product_selector( $email ) {
		...
		if ( in_array( $email->type, array( 'fooevents' ) ) ) {
	...
	public function after_create_ticket( $ticket_post_id ) {
		...
		foreach ( $emails as $email ) {
			...
			$send = false;
			if ( ! empty( $attendee_email ) && ( ( ! empty( $email->product_id ) && empty( $email->category_id ) && $product_id === $email->product_id ) || ( ! empty( $email->category_id ) && empty( $email->product_id ) && in_array( $email->category_id, $product_cats_ids ) ) || ( ! empty( $email->product_id ) && $product_id === $email->product_id ) && ( ! empty( $email->category_id ) && in_array( $email->category_id, $product_cats_ids ) ) ) ) {
	...
	public function before_after_event_date( $ticket_post_id ) {
		...
		foreach ( $emails as $email ) {
			...
			$send = false;
			if ( ! empty( $attendee_email ) && in_array( $event_type, array( 'single', 'sequential', 'select' ), true ) && ! empty( $event_date_timestamp ) && ( ( ! empty( $email->product_id ) && empty( $email->category_id ) && $product_id === $email->product_id ) || ( ! empty( $email->category_id ) && empty( $email->product_id ) && in_array( $email->category_id, $product_cats_ids ) ) || ( ! empty( $email->product_id ) && $product_id === $email->product_id ) && ( ! empty( $email->category_id ) && in_array( $email->category_id, $product_cats_ids ) ) ) ) {

				$send = true;

			} elseif ( empty( $email->product_id ) && empty( $email->category_id ) && ! empty( $attendee_email ) && in_array( $event_type, array( 'single', 'sequential', 'select' ), true ) && ! empty( $event_date_timestamp ) ) {
			...
			if ( true === $send ) {
				...
				if ( 'before_fooevents_event' === $email->trigger ) {
					...
					if ( in_array( $event_type, array( 'sequential', 'select' ), true ) ) {
	...
	public function after_check_in_ticket( $args ) {
		...
		foreach ( $emails as $email ) {
			...
			$send = false;
			if ( ! empty( $attendee_email ) && ! empty( $timestamp ) && in_array( $status, array( 'Checked In' ), true ) && ( ( ! empty( $email->product_id ) && empty( $email->category_id ) && $product_id === $email->product_id ) || ( ! empty( $email->category_id ) && empty( $email->product_id ) && in_array( $email->category_id, $product_cats_ids ) ) || ( ! empty( $email->product_id ) && $product_id === $email->product_id ) && ( ! empty( $email->category_id ) && in_array( $email->category_id, $product_cats_ids ) ) ) ) {

				$send = true;

			} elseif ( empty( $email->product_id ) && empty( $email->category_id ) && ! empty( $attendee_email ) && ! empty( $timestamp ) && in_array( $status, array( 'Checked In' ), true ) ) {
