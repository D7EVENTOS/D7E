...
class FooEvents_Event_Report_Helper extends WP_List_Table {
	...
	public function display_report_page() {
		...
		$id    = sanitize_text_field( wp_unslash( $_GET['event'] ) );
		$event = get_post( $id );
		...
		if ( isset( $_POST['WooCommerceEventsDateFrom'] ) ) {
			...
			$previous_date = sanitize_text_field( wp_unslash( $_POST['WooCommerceEventsDateFrom'] ) );

		}

		if ( isset( $_POST['WooCommerceEventsDateTo'] ) ) {

			$todays_date = sanitize_text_field( wp_unslash( $_POST['WooCommerceEventsDateTo'] ) );
	...
	public function fetch_check_ins_today() {
		...
		$id             = sanitize_text_field( wp_unslash( $_POST['eventID'] ) );
	...
	public function fetch_tickets_sold() {
		...
		$canceled_tickets = sanitize_text_field( $_POST['canceledTickets'] );
