	...
	public function generate_ics( $event, $ticket_id, $registrant_email = '' ) {
		...
		if ( 'select' === $multi_day_type ) {
			...
			foreach ( $woocommerce_events_select_date as $day_date ) {
				...
				$temp_start_date = DateTime::createFromFormat( $date_format . $start_period_format, $day_date . ' ' . $woocommerce_events_select_date_hour[ $x ] . ':' . $woocommerce_events_select_date_minutes[ $x ] . $woocommerce_events_select_date_period[ $x ] );
				$temp_end_date   = DateTime::createFromFormat( $date_format . $end_period_format, $day_date . ' ' . $woocommerce_events_select_date_hour_end[ $x ] . ':' . $woocommerce_events_select_date_minutes_end[ $x ] . $woocommerce_events_select_date_period_end[ $x ] );
			...
			if ( isset( $ics_data['OffsetEndHours'] ) && ! empty( $ics_data['OffsetEndHours'] ) ) {
				...
				$end_date = date( 'Y-m-d H:i:s', strtotime( '+' . $ics_data['OffsetEndHours'] . ' hours +' . $ics_data['OffsetEndMinutes'] . ' minutes', strtotime( $endDate ) ) );
