...
class FooEvents_Zoom_API_Helper {
	...
	public function fooevents_zoom_test_access() {
		...
		$key    = isset( $_POST['key'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['key'] ) ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$secret = isset( $_POST['secret'] ) ? trim( sanitize_text_field( wp_unslash( $_POST['secret'] ) ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		$result = $this->fooevents_zoom_request( 'users/me', array(), $jwt );
	...
	public function fooevents_zoom_fetch_users() {
		...
		if ( ! empty( $_POST['key'] ) && '' !== trim( sanitize_text_field( wp_unslash( $_POST['key'] ) ) ) && ! empty( $_POST['secret'] ) && '' !== trim( sanitize_text_field( wp_unslash( $_POST['secret'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing

			$key    = trim( sanitize_text_field( wp_unslash( $_POST['key'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$secret = trim( sanitize_text_field( wp_unslash( $_POST['secret'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing

		}

		if ( '' === trim( $key ) || '' === trim( $secret ) ) {

			// Check for saved key and secret.
			$key    = get_option( 'globalWooCommerceEventsZoomAPIKey', '' );
			$secret = get_option( 'globalWooCommerceEventsZoomAPISecret', '' );
