...
class FooEvents {
	...
	public function check_fooevents_errors() {
		...
		if ( ! empty( $_GET['fooevents_error'] ) ) {
			...
			$error_code = sanitize_text_field( wp_unslash( $_GET['fooevents_error'] ) );
	...
	public function fooevents_wpml_compatibility( $wp_query ) {
		...
		/*
		if ( ! empty( $_GET['action'] ) && 'woocommerce_events_csv' === $_GET['action'] ) {
	...
	public function register_scripts() {
		...
		if ( isset( $_GET['page'] ) && 'fooevents-settings' === $_GET['page'] ) {

			wp_enqueue_media();

		}

		if ( isset( $_GET['page'] ) && 'fooevents-event-report' === $_GET['page'] ) {
	...
	public function register_styles() {
		...
		if ( ( isset( $_GET['post'] ) && isset( $_GET['action'] ) && 'edit' === $_GET['action'] ) || ( isset( $_GET['page'] ) && 'fooevents-event-report' === $_GET['page'] ) ) {
	...
	public function fooevents_menu_order( $menu_ord ) {
		...
		if ( ! is_network_admin() ) {
			...
			if ( isset( $submenu['fooevents'][7] ) ) {

				$menu[] = $submenu['fooevents'][7];
			...
			if ( is_plugin_active( 'fooevents_express_check_in/fooevents-express-check_in.php' ) || is_plugin_active_for_network( 'fooevents_express_check_in/fooevents-express-check_in.php' ) ) {
				...
				if ( isset( $submenu['fooevents'][6] ) ) {

					$menu[] = $submenu['fooevents'][6];
	...
	public function fooevents_ics() {
		...
		if ( isset( $_GET['ticket'] ) ) {
			...
			$ticket_id = sanitize_text_field( wp_unslash( $_GET['ticket'] ) );
		...
		if ( isset( $_GET['email'] ) ) {
			...
			$registrant_email = isset( $_GET['email'] ) ? sanitize_text_field( wp_unslash( $_GET['email'] ) ) : '';
	...
	public function fooevents_filter_ticket_results( $query ) {
		...
		if ( is_admin() && 'edit.php' === $pagenow && isset( $_GET['event_id'] ) && '' !== $_GET['event_id'] ) {
			...
			$fooevents_filter                = sanitize_text_field( wp_unslash( $_GET['event_id'] ) );
